...
void SetUp(Player &player1, Player &player2, Player &player3,
	 	map<string,string> tile, vector<string> &wallhand, vector<string> sortby){
...
void ContinueToBookmakerOrChange(int &continuetobookmaker, int winner,
		bool &player1bookmaker, bool &player2bookmaker, bool &player3bookmaker,
...
void DiscardAndPrintHand(vector<string> sortby, map<string,string> tile, 
		int &gameround, vector<string> &riverhand, vector<string> wallhand,
...
ReStart:
	...
	while (gameloop) {
		...
		// player3 discard and other player check tsumo, ron or meld
		if (gameturn == 0) {
			...
			WinAndMeldCheck(sortby, tile, riverhand, wallhand, player2, gameturn, 
					gameloop,	false, top, tsumo, eye, gameflow);
			if (winnerortempgameturn == gameturn)
	...
	s.PrintScore();
	if (player3.bookmaker && winnerortempgameturn != 3) {
		return  0;
	}else{
		for(int i = 6; i > 0; i--){
			cout << i << "秒後開始下一輪" << endl; 
		...
		ContinueToBookmakerOrChange(continuetobookmaker, winnerortempgameturn, 
				player1.bookmaker, player2.bookmaker, player3.bookmaker, tempbookmaker,
			 	gameturn);
		tempbookmaker = 0;
		...
		pa.PrintAll_SetNameAndMap(player1.name, player2.name, player3.name, "牌山",
			 	"牌河", tile);
		// cw class set tile
		cw.CheckWin_SetTile(sortby);
		// cm class set tile
